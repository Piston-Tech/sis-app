...
export default function AdminFoundationPage() {
  ...
  async function toggleProgramStatus(program: Program) {
    ...
    if (response.ok) {
      setMessage(`Program ${!program.isActive ? "activated" : "deactivated"}.`);
      await load();
      return;
    }
  ...
  async function updateApplicationStatus(
    ...
    const response = await fetch(
      `/api/admin/foundation/applications/${applicationId}`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-zinc-500">
          Manage foundation programs, rolling cohorts, and referral
          applications.
        </p>
      </header>
