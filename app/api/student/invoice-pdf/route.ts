...
export async function GET() {
  try {
    ...
    const { data: enrollData } = await apiServer<{ data?: EnrollmentData[] }>({
    ...
    const totalBilled = transactionGroups.reduce(
      (sum, group) => sum + group.totalCost,
      0,
    );
    const totalPaid = transactionGroups.reduce(
    ...
    const pdfBuffer = await new Promise<Buffer>((resolve, reject) => {
      ...
      if (!transactionGroups.length) {
        ...
        transactionGroups.forEach((group, index) => {
          ...
          if (
            txPayerType.includes("company") ||
            txPayerType.includes("corporate")
          ) {
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="invoice-receipt-${String(student?.studentId || "student").replace(/[^A-Za-z0-9_-]/g, "")}.pdf"`,
