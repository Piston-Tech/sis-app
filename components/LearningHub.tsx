...
const MOCK_COURSES: Array<{
...
}> = [
  {
    ...
    progress: 78,
    thumbnail:
      "https://images.unsplash.com/photo-1507925921958-8a62f3d1a50d?auto=format&fit=crop&q=80&w=400",
    tierRequired: MembershipTier.BASIC,
    segments: [UserRole.PROFESSIONAL, UserRole.JOB_SEEKER],
  },
  {
    id: 2,
    title: "Advanced Business Analysis (IIBA)",
    category: "Business Analysis",
    progress: 42,
    thumbnail:
      "https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&q=80&w=400",
    tierRequired: MembershipTier.PRO,
    segments: [UserRole.PROFESSIONAL],
  },
  {
    id: 3,
    title: "SME Scaling & Operational Excellence",
    category: "Entrepreneurship",
    progress: 0,
    thumbnail:
      "https://images.unsplash.com/photo-1553729459-efe14ef6055d?auto=format&fit=crop&q=80&w=400",
    tierRequired: MembershipTier.ELITE,
    segments: [UserRole.SME_OWNER],
  },
  {
    id: 4,
  ...
  },
  {
    ...
    tierRequired: MembershipTier.ELITE,
    segments: [UserRole.CORPORATE_ADMIN, UserRole.PROFESSIONAL],
  },
  {
    id: 6,
    ...
    tierRequired: MembershipTier.PRO,
    segments: [UserRole.SME_OWNER, UserRole.CORPORATE_ADMIN],
  },
];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Personalized training tracks for your {user.role.replace("_", " ")}{" "}
            journey.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {filteredCourses.map((course) => {
          ...
          return (
            <div
              key={course.id}
              className={`group relative bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col ${locked ? "opacity-75" : ""}`}
            >
              <div className="relative h-52 overflow-hidden">
                <img
                  src={course.thumbnail}
                  alt={course.title}
                  className={`w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ${locked ? "grayscale" : ""}`}
                />
              ...
              <div className="p-8 flex-1 flex flex-col">
                <h3 className="text-xl font-black text-slate-900 tracking-tight leading-tight mb-4">
                  {course.title}
                </h3>
                <div className="mt-auto">
                  ...
                  {locked ? (
                    <button className="w-full py-4 bg-slate-100 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest cursor-not-allowed">
                      Upgrade to Access
                    </button>
                  ) : (
